...
import { CONFIG_FILENAMES, findConfigFile, toPosix } from "@glosik/core";
import { Command } from "commander";

const TEMPLATE = `import { defineConfig } from "@glosik/schema";
...
export default defineConfig({
  ...
  // fable-5, opus-4.7/4.8) reject sampling parameters with a 400, so glosik
...
export const runInit = async (root: string, force: boolean): Promise<string> => {
  ...
  const target = path.resolve(root, CONFIG_FILENAMES[0] ?? "glosik.config.ts");
